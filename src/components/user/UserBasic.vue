  ...
  export default {
    ...
    methods: {
      ...
      editUser() {
        ...
        this.$store.dispatch('updateUserById', obj)
          .then(res => {
            if (res.code === '0') {
              this.dialogVisible = false;
              this.initUserList()
      ...
      changePWD() {
        let obj = {
          'id': this.ruleForm.id,
          'oldPassword': Encrypt(this.ruleForm.oldPass).toString(),
          'firstPassword': Encrypt(this.ruleForm.pass).toString(),
          'secPassword': Encrypt(this.ruleForm.checkPass).toString()
        };
